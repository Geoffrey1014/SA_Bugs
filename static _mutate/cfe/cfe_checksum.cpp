...
static llvm::cl::OptionCategory MyToolCategory("Checksum transformer");

void InitTool() {
    FlagPreFix.addCategory(MyToolCategory);
    Stub.addCategory(MyToolCategory);
    FuncWrap.addCategory(MyToolCategory);
}


class C2Transformer : public BranchVisitor<C2Transformer, true> {
private:
...
public:
    ...
    bool TraverseVarDecl(VarDecl *V) {
        if (V->getName().contains(FlagPreFix.getValue())) {
            caut_errs() << "existing branch stub " << V->getName() << " defined";
            return false;
        }
...
return false; \
            ...
            if (find(exit_points, func_name, t)) {
                // exit(i_exp) => stub(); if ((i_exp) != 0) { err(); } else {};
                CHECK_ARGC(1, "exit")
                prep_rep = STUB_FUNC_CALL_STMT(Stub) + " if ( ";
                postp_rep = std::string(" != 0) {") + STUB_FUNC_CALL_STMT(ErrStub) + " } else {}";
...
int main(int argc, const char **argv) {
    ...
    return Tool.run(newFrontendActionFactory<TransformAction<C2Transformer>>().get());
}
